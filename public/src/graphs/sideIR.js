...
export default function({ container, x, width, height, offsetTop }) {
  ...
  let frontLeftIR = svg.append("path")
      .style("clip-path", "url(#clip-ir_side)")
      .attr("class", "ir_side");
  ...
  function redraw() {
    ...
    let leftIR  = originalData.map(d => ({ x: x(d.time), y: y(d.ir_side[0])}));
    let rightIR = originalData.map(d => ({ x: x(d.time), y: y(-d.ir_side[1])}));
